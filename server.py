...
def _get_fb_access_token() -> str:
    """
    Get Facebook access token from command line arguments or environment variable.
    ...
    Raises:
        Exception: If no token is provided in command line arguments or environment variable.
    """
        ...
        if "--fb-token" in sys.argv:
            try:
                ...
                if token_index < len(sys.argv):
                    ...
                    sys.stderr.write("Using Facebook token from command line arguments\n")
                else:
                    raise Exception("--fb-token argument provided but no token value followed it")
            except ValueError:
                 pass # Should not happen given the check, but safe to ignore
        
        # If still None, look for environment variable
        if FB_ACCESS_TOKEN is None:
             import os
             FB_ACCESS_TOKEN = os.environ.get("FB_ACCESS_TOKEN")
             if FB_ACCESS_TOKEN:
                 sys.stderr.write("Using Facebook token from environment variable\n")

        # If still None, look for .env file manually
        if FB_ACCESS_TOKEN is None:
            try:
                    ...
                    with open(".env", "r") as f:
                        for line in f:
                            if line.startswith("FB_ACCESS_TOKEN="):
                                ...
                                sys.stderr.write("Using Facebook token from .env file\n")
                                break
            except Exception:
                pass
...
def _make_graph_api_call(url: str, params: Dict[str, Any]) -> Dict:
    ...
    try:
        ...
        # If not successful, try to extract detailed FB error info
        if response.status_code != 200:
            try:
                ...
                # We raise a custom Exception with the detailed message
                raise Exception(full_error_msg)
            except (ValueError, KeyError, AttributeError):
                # Fallback if body is not JSON or doesn't have expected structure
                sys.stderr.write(f"FB API Error Body: {response.text}\n")
                response.raise_for_status()

        return response.json()
    except requests.exceptions.RequestException as e:
        # Log the error to stderr and re-raise
        sys.stderr.write(f"Error making Graph API call to {url} with params {params}: {e}\n")
        raise
